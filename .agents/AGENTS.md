...
*   **`/okc-bookFlashcards <input> [flags]`** / **`/okc-bookDeck <input> [flags]`**
    *   *Purpose:* Transform an entire EPUB or PDF non-fiction book into an exhaustive active-recall flashcard deck using the **HDAR (*High-Density Active Recall*)** standard. Decomposes into semantic sections, extracts core mechanisms and trade-offs, formulates atomic `Basic (optional reversed card)` notes, avoids duplicates, embeds diagrams, and syncs directly to Anki and Obsidian.
    *   *Execution:* Run `uv run python scripts/book_flashcards_engine.py --input "<input>" --deck "<DeckName>" [--sync-anki] [--replace-deck]`.
